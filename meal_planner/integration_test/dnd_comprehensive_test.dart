...
void main() {
  ...
  DateTime getDate(int daysFromNow) {
  ...
  Future<void> resetCalendar(WidgetTester tester) async {
  ...
  Future<void> addMeal(WidgetTester tester, DateTime day) async {
  ...
  String getActivityCount(WidgetTester tester) {
  ...
  group('Test Suite 1: Basic Add/Remove Operations', () {
    testWidgets('Test 1.1: Add Meal to Empty Day', (tester) async {
      app.main();
      await tester.pumpAndSettle();
      await takeScreenshot(tester, 'test_start_state.png');

      final initialCount = getActivityCount(tester);
      final today = DateTime.now();
      
      await addMeal(tester, today);
      await takeScreenshot(tester, 'test_1_1_add_meal.png');

      final newCount = getActivityCount(tester);
    ...
    testWidgets('Test 1.2: Remove Meal with Delete Button', (tester) async {
      ...
      final dayKey = dateKey(today);
      final deleteButton = find.byKey(Key('delete-meal-$dayKey-0'));
      
      if (deleteButton.evaluate().isNotEmpty) {
        final beforeCount = getActivityCount(tester);
        await tester.tap(deleteButton);
        await tester.pumpAndSettle();
        await takeScreenshot(tester, 'test_1_2_remove_meal.png');

        final afterCount = getActivityCount(tester);
    ...
    testWidgets('Test 1.3: Reset All Meals', (tester) async {
      ...
      await addMeal(tester, DateTime.now());
      await addMeal(tester, getDate(1));
      await addMeal(tester, getDate(2));
      await tester.pumpAndSettle();
      await takeScreenshot(tester, 'test_1_3_before_reset.png');

      expect(getActivityCount(tester), contains('3'));

      await resetCalendar(tester);
      await takeScreenshot(tester, 'test_1_3_after_reset.png');

      expect(getActivityCount(tester), equals('Total: 0 activities'));
    });
  });
  ...
  group('Test Suite 2: Drag-and-Drop Between Adjacent Days', () {
    testWidgets('Test 2.1: Drag from Day with Event to Empty Day', (tester) async {
      ...
      final dayX = getDate(0);
      final dayX1 = getDate(1);
      
      await addMeal(tester, dayX);
      await tester.pumpAndSettle();
      await takeScreenshot(tester, 'test_2_1_before_drag.png');

      final dayXKey = dateKey(dayX);
      final dayX1Key = dateKey(dayX1);
      ...
      if (mealCard.evaluate().isNotEmpty && dragTarget.evaluate().isNotEmpty) {
        ...
        await tester.pumpAndSettle();
        await takeScreenshot(tester, 'test_2_1_after_drop.png');
    ...
    testWidgets('Test 2.2: Drag to combine meals on same day', (tester) async {
      ...
      final dayX = getDate(2);
      final dayX1 = getDate(3);
      
      await addMeal(tester, dayX);
      await addMeal(tester, dayX1);
      await tester.pumpAndSettle();
      await takeScreenshot(tester, 'test_2_2_before_combine.png');
      ...
      if (mealCard.evaluate().isNotEmpty) {
        ...
        await tester.pumpAndSettle();
        await takeScreenshot(tester, 'test_2_2_after_combine.png');

        expect(getActivityCount(tester), contains('2'));
      }
    });
  });
  ...
  group('Test Suite 3: Empty Day Drop Zone Validation', () {
    testWidgets('Test 3.1: Empty Day is Valid Drop Target', (tester) async {
      ...
      if (mealCard.evaluate().isNotEmpty && emptyDayTarget.evaluate().isNotEmpty) {
        ...
        await tester.pumpAndSettle();
        await takeScreenshot(tester, 'test_3_1_empty_hover.png');
  ...
  group('Test Suite 4: Week Number and Scroll Synchronization', () {
    testWidgets('Test 4.1: Week Number Updates on Scroll', (tester) async {
      app.main();
      await tester.pumpAndSettle();
      await takeScreenshot(tester, 'test_4_1_week_42.png');
      ...
      await tester.pumpAndSettle();
      await takeScreenshot(tester, 'test_4_1_week_43.png');
  ...
  group('Test Suite 5: Visual Feedback Validation', () {
    testWidgets('Test 7.1: Drag Feedback Visual Check', (tester) async {
      ...
      await takeScreenshot(tester, 'test_7_1_drag_feedback.png');
    ...
    testWidgets('Test 7.3: Quick Meal Lightning Icon', (tester) async {
      ...
      await addMeal(tester, dayX);
      await tester.pumpAndSettle();
      await takeScreenshot(tester, 'test_7_3_lightning_icons.png');
  ...
  group('Test Suite 6: State Persistence', () {
    testWidgets('Test 8.1: State Survives Multiple Operations', (tester) async {
      ...
      await addMeal(tester, getDate(1));
      await addMeal(tester, getDate(1));
      await addMeal(tester, getDate(3));
      await addMeal(tester, getDate(3));
      await addMeal(tester, getDate(3));
      await addMeal(tester, getDate(4));
      await tester.pumpAndSettle();
      await takeScreenshot(tester, 'test_8_1_state_persistence.png');

      expect(getActivityCount(tester), equals('Total: 6 activities'));
    ...
    testWidgets('Test 8.2: Reset Clears All State', (tester) async {
      ...
      for (int i = 0; i < 10; i++) {
        await addMeal(tester, getDate(i));
      }
      await tester.pumpAndSettle();
      await takeScreenshot(tester, 'test_8_2_before_reset.png');

      expect(getActivityCount(tester), contains('10'));

      await resetCalendar(tester);
      await takeScreenshot(tester, 'test_8_2_after_reset.png');
      ...
      expect(getActivityCount(tester), equals('Total: 1 activities'));
    });
  });
  ...
  group('Test Suite 7: Alignment Validation', () {
    testWidgets('Test 9.1: Day Header and Card Alignment', (tester) async {
      ...
      await addMeal(tester, getDate(0));
      await addMeal(tester, getDate(2));
      await addMeal(tester, getDate(4));
      await tester.pumpAndSettle();
      await takeScreenshot(tester, 'test_9_1_alignment.png');
    ...
    testWidgets('Test 9.2: Empty Day Layout', (tester) async {
      ...
      await tester.pumpAndSettle();
      await resetCalendar(tester);
      await takeScreenshot(tester, 'test_9_2_empty_layout.png');
  ...
  group('Test Suite 8: Complete Integration', () {
    testWidgets('Test 10.1: Complete User Workflow', (tester) async {
      ...
      await tester.pumpAndSettle();
      await resetCalendar(tester);
      await takeScreenshot(tester, 'e2e_1_open.png');

      await addMeal(tester, getDate(0));
      await addMeal(tester, getDate(2));
      await addMeal(tester, getDate(4));
      await tester.pumpAndSettle();
      await takeScreenshot(tester, 'e2e_2_initial_plan.png');
      expect(getActivityCount(tester), equals('Total: 3 activities'));

      final day0Key = dateKey(getDate(0));
      final mealCard = find.byKey(Key('meal-New Meal-$day0Key'));
      
      if (mealCard.evaluate().isNotEmpty) {
        await tester.drag(mealCard, const Offset(0, 600));
        await tester.pumpAndSettle();
        await takeScreenshot(tester, 'e2e_3_rearranged.png');
        expect(getActivityCount(tester), equals('Total: 3 activities'));
      }

      await addMeal(tester, getDate(1));
      await addMeal(tester, getDate(5));
      await tester.pumpAndSettle();
      await takeScreenshot(tester, 'e2e_4_added_more.png');
      expect(getActivityCount(tester), equals('Total: 5 activities'));

      final day1Key = dateKey(getDate(1));
      final deleteButton = find.byKey(Key('delete-meal-$day1Key-0'));
      if (deleteButton.evaluate().isNotEmpty) {
        await tester.tap(deleteButton);
        await tester.pumpAndSettle();
        await takeScreenshot(tester, 'e2e_5_removed.png');
        expect(getActivityCount(tester), equals('Total: 4 activities'));
      }

      final eventsList = find.byType(EventsList);
      await tester.fling(eventsList, const Offset(0, -1000), 1000);
      await tester.pumpAndSettle();
      await takeScreenshot(tester, 'e2e_6_week_43.png');

      await tester.fling(eventsList, const Offset(0, 1000), 1000);
      await tester.pumpAndSettle();
      await takeScreenshot(tester, 'e2e_7_review.png');

      await resetCalendar(tester);
      await takeScreenshot(tester, 'e2e_8_reset.png');
      expect(getActivityCount(tester), equals('Total: 0 activities'));
    });
  });
}
