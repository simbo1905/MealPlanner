<script lang="ts">
  ...
  const handleSelectMeal = (meal: any) => {
    console.log(`➕ ADD MEAL: "${meal.name}" → DATE: ${selectedDate}`);
    onAddActivity(selectedDate, meal);
    console.log(`➕ ADDED: "${meal.name}" successfully`);
  };

  // Action panel state
  let actionPanelMealId = $state<string | null>(null);

  const toggleActionPanel = (meal: any, date: string, e: Event) => {
    console.log(`⚡ LIGHTNING CLICK: meal="${meal.name}" id=${meal.id} date=${date}`);
    e.stopPropagation();
    actionPanelMealId = actionPanelMealId === meal.id ? null : meal.id;
    console.log(`⚡ PANEL STATE: ${actionPanelMealId ? `OPEN for ${meal.id}` : 'CLOSED'}`);
  };

  const handleDeleteMeal = (date: string, meal: any) => {
    console.log(`🗑️  DELETE CLICK: meal="${meal.name}" id=${meal.id} date=${date}`);
    const dayIndex = week.days.findIndex((day: Day) => day.date === date);
    if (dayIndex !== -1) {
      const beforeCount = week.days[dayIndex].meals?.length || 0;
      const beforeMeals = week.days[dayIndex].meals?.map(m => m.name).join(', ') || 'none';
      
      week.days[dayIndex].meals = week.days[dayIndex].meals?.filter(m => m.id !== meal.id) || [];
      week.days[dayIndex].activities = week.days[dayIndex].meals?.length || 0;
      ...
      const afterCount = week.days[dayIndex].meals?.length || 0;
      const afterMeals = week.days[dayIndex].meals?.map(m => m.name).join(', ') || 'none';
      console.log(`🗑️  DELETED: "${meal.name}" | Before: ${beforeCount} [${beforeMeals}] → After: ${afterCount} [${afterMeals}]`);
    }
    actionPanelMealId = null;
  };

  const handleCancelAction = () => {
    console.log('❌ CANCEL: Action panel closed without deleting');
    actionPanelMealId = null;
  };
            ...
            >
              {#each day.meals || [] as meal (meal.id)}
                <div 
                  ...
                  class="flex-shrink-0 w-52 p-4 bg-white rounded-xl shadow-sm border border-gray-100 {getBorderColor(meal.color)} border-l-4 cursor-move transition-all hover:shadow-md active:scale-[0.98] scroll-snap-align-start relative"
                >
                  <div class="flex items-start justify-between">
                    ...
                    <button 
                      onclick={(e) => toggleActionPanel(meal, day.date, e)}
                      class="hover:bg-gray-100 rounded p-1 transition-colors cursor-pointer"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-zap w-4 h-4 text-gray-400 flex-shrink-0">
                        <path d="M4 14a1 1 0 0 1-.78-1.63l9.9-10.2a.5.5 0 0 1 .86.46l-1.92 6.02A1 1 0 0 0 13 10h7a1 1 0 0 1 .78 1.63l-9.9 10.2a.5.5 0 0 1-.86-.46l1.92-6.02A1 1 0 0 0 11 14z"></path>
                      </svg>
                    </button>
                  </div>

                  {#if actionPanelMealId === meal.id}
                    {@const _ = console.log('🎯 Rendering action panel for meal:', meal.id)}
                    <div class="absolute top-full left-0 right-0 mt-2 bg-white rounded-lg shadow-lg border border-gray-200 p-2 flex gap-2 z-30">
                      <button 
                        onclick={() => handleDeleteMeal(day.date, meal)}
                      ...
                      <button 
                        ...
                        class="flex-1 px-3 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 transition-colors text-sm font-medium"
                      >
                        Cancel
                      </button>
                    </div>
