...
const String recipesSubCollection = 'recipes';
...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final recipes = lines
        ...
        .map((title) => {'title': title.trim(), 'createdAt': FieldValue.serverTimestamp()})
        .toList();
    ...
    for (int i = 0; i < recipes.length; i += batchSize) {
      ...
      for (int j = i; j < end; j++) {
        final docRef = firestore.collection(collectionName).doc(recipesSubCollection).collection('recipes').doc();
    ...
    print('✓ Successfully uploaded ${recipes.length} recipes to $collectionName/$recipesSubCollection');
  } catch (e) {
